...
class Experiment(object):
    ...
    def __init__(self, X, y, train_set_ratios, FOLDTORUN, splitter, EVALUATION_MEASURES, 
                 METHODNAMES, METHODS, METHODNAMESSINGLETASK, METHODSSINGLETASK, print_metrics,
    ...
    def run(self):
        for foldind, (train, test) in enumerate(self.splitter):
            if self.FOLDTORUN==-1 or self.FOLDTORUN==foldind:
                ...
                settings_dict={}
                settings_dict['multi']=(self.X[train, :], self.X[test, :], self.y[train], self.y[test], 
                            self.METHODNAMES, 
                            self.METHODS)
                settings_dict['single']=(self.X[train, :][self.X[train][:,self.POSTPROCESSED_TASK_COLUMN_ID]==
